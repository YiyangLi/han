...
- `src/poller.ts` — Finnhub news poller: `fetchAndStoreNews()` (one cycle: fetch `category=business`, keep only articles whose own `category` is `business`, insert via `createMany` + `skipDuplicates` deduped by the unique `finnhubId`) and `startNewsPoller()` (immediate run + 60s interval, in-flight guard). Required-field checks use `!= null` (not `!== undefined`) so a single article with an explicit `null` field is skipped instead of poisoning the whole `createMany` batch. Started only from the `require.main` block in `index.ts` when `FINNHUB_TOKEN` is set — never from `createApp()`, so tests don't poll. Tests mock global `fetch` and the prisma singleton.
